...
import { grantPluginPermission } from "@/server/plugins/permissions";
...
const schema = z.object({
  ...
  playerName: z.string().trim().max(160).optional().nullable(),
  pluginName: z.string().trim().max(160).optional().nullable(),
});
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const result = await grantPluginPermission(
      server,
      parsed.data.steamId,
