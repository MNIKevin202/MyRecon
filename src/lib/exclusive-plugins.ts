...
export const EXCLUSIVE_PLUGINS: ExclusivePlugin[] = [
  ...
  },
  {
    ...
    name: "NoClip",
    version: "1.0.0",
    description: "Let permitted players toggle noclip (fly through walls and terrain) with /noclip.",
    longDescription: "Grants a /noclip chat command (and myrcon.noclip console command) to players with the myrconnoclip.use permission. Non-admins are temporarily elevated so the client accepts the noclip command, then reverted automatically when they toggle it off or disconnect — no permanent admin flag is left behind. Grant the permission from the Exclusive Plugins → Manage screen.",
    tags: ["Admin", "Movement", "QoL"],
    filename: "MyRconNoClip.cs",
    defaultPath: "carbon/plugins/MyRconNoClip.cs",
    permissions: ["myrconnoclip.use"],
    previewItems: ["tool.camera", "jackhammer", "metal.facemask", "wood"],
    contentUrl: "https://raw.githubusercontent.com/MNIKevin202/MyRecon/main/plugins/MyRconNoClip.cs",
  },
  {
    id: "structure-iq",
    name: "StructureIQ",
